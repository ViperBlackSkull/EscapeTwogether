...
| **Clock Hands** | `clock-hands.png` | Clock Face, Midnight Chime puzzles |
| **Gears Collection** | `gears-collection.png` | Music Box, Gear Alignment puzzles |
| **Telegraph Key** | `telegraph-key.png` | Bell Codes puzzle tapping interface |
| **Botanical Illustration** | `botanical-illustration.png` | Garden puzzles, Hybridization |
...
### Puzzle-to-Asset Mapping (Room 2 - Implemented in Logic Files)

| Puzzle | Logic File | Assets Used |
|--------|------------|-------------|
| Bell Codes | `bellCodesPuzzle.ts` | `codebook`, `mysteriousGlyphs`, **`telegraphKey`**, `victorianOrnament`, `hourglass` |
| Clock Face | `clockFacePuzzle.ts` | `compass`, `hourglass`, **`clockHands`**, `victorianOrnament`, `candleHolder` |
| Gear Alignment | `gearAlignmentPuzzle.ts` | **`gearsCollection`**, `antiqueKeys`, `compass`, `hourglass` |
| Windup Keys | `windupKeyPuzzle.ts` | `antiqueKeys`, `codebook` |
| Pendulum Maze | `pendulumPuzzle.ts` | `compass`, `hourglass` |
| Midnight Chime | `midnightChimePuzzle.ts` | `hourglass`, `clockHands` |

### Puzzle-to-Asset Mapping (Room 3 - Implemented in Logic Files)

| Puzzle | Logic File | Assets Used |
|--------|------------|-------------|
| Seed Packets | `seed-packets.ts` | `treasureMap`, `codebook` |
| Water Flow | `water-flow.ts` | `compass`, `cipherWheel` |
| Light Spectrum | `light-spectrum.ts` | `cipherWheel` |
| Hybridization | `hybridization.ts` | **`botanicalIllustration`**, `treasureMap`, `cluePhotograph` |
| Trellis | `trellis.ts` | `victorianOrnament`, `mysteriousGlyphs` |
| Final Bloom | `final-bloom.ts` | `clueLetter`, `cluePhotograph` |
